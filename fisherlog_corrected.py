...
print(cwd)
str_cwd=cwd.replace("/"+remove+"/"+version,"")
print(str_cwd)
...
ylog = np.array([df_train_log['AddedLogvalue']]).T
...
for i in range(0,len(x[0])):
	...
	try:	
		...
		if n_cs==0 and n_cf==n_f:
			n_cs=1
		factor2=ylog[n_cs-1]+ylog[n_cf-1]+ylog[n_us-1]+ylog[n_uf-1]
...
csvfile=open(str_cwd+"/FisherjournlogMod.csv", "a+")
